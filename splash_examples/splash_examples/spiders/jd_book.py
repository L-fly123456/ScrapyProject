...
function main(splash)
    ...
    splash:wait(2)
    splash:runjs("ducoment.getElementsByClassName('page')[0].scrollIntoView(true)")
    splash:wait(2)
    return splash:html()
end
'''
...
class JdBookSpider(scrapy.Spider):
    ...
    def parse_urls(self,response):
        # 获取商品总数，计算出总页数
        # total = int(eval(response.css('span#J_resCount::text').extract_first().replace("万+","")))
        total = 650
        pageNUM = total // 60 +(1 if total % 60 else 0)

        # 构造每页的url，向Splash 的execute端点发送请求
